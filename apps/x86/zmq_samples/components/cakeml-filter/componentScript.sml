...
val _ = declare_virtqueue_dev "vq_vm0_recv";
val _ = declare_virtqueue_drv "vq_vm0_send";
val _ = declare_virtqueue_dev "vq_vm1_recv";
val _ = declare_virtqueue_drv "vq_vm1_send";
...
val post_init_def = (append_prog o process_topdecs) `
    fun post_init u = List.app VirtQueue.unwrap [
        VirtQueue.driver_init vq_vm0_send 0,
        VirtQueue.device_init vq_vm0_recv 1,
        VirtQueue.driver_init vq_vm1_send 2,
        VirtQueue.device_init vq_vm1_recv 3
    ]
`;

val destination_vq_def = (append_prog o process_topdecs) `
    fun destination_vq sent_from =
        if sent_from = vq_vm0_recv then
            vq_vm1_send
        else (* if sent_from = vq_vm1_recv then *)
            vq_vm0_send
`;
...
val callback_def = (append_prog o process_topdecs) `
    fun callback badge = let
        fun forward_message vq = case vq of
            ...
            VirtQueue.VirtQueueDev _ => (case VirtQueue.device_recv vq of
                VirtQueue.Ok message => let
                    ...
                    val destination = destination_vq vq;
        ...
        |   VirtQueue.VirtQueueDrv _ => let
                ...
                in () end
        in select_loop forward_message [vq_vm0_recv, vq_vm0_send, vq_vm1_recv, vq_vm1_send] end
`;
